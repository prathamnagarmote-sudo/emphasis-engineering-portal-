...
const HeroOrbs = () => (
  <div className="absolute inset-0 overflow-hidden pointer-events-none">
    <motion.div animate={{ scale: [1, 1.3, 1], opacity: [0.3, 0.5, 0.3] }} transition={{ duration: 7, repeat: Infinity }}
      className="absolute -top-20 -left-20 w-80 h-80 bg-primary/30 rounded-full blur-[80px]" />
    <motion.div animate={{ scale: [1, 1.2, 1], opacity: [0.2, 0.4, 0.2] }} transition={{ duration: 9, repeat: Infinity, delay: 1 }}
      className="absolute bottom-10 right-0 w-96 h-96 bg-teal-400/20 rounded-full blur-[100px]" />
...
// ─── OTP Step ──────────────────────────────────────────────────────────────────
const OtpStep = ({ email, onVerified, onBack }: { email: string; onVerified: (code?: string) => void; onBack: () => void }) => {
  ...
  return (
    <motion.div initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }}>
      <div className="text-center mb-8">
        ...
        <p className="text-gray-500 text-sm mt-2">Please enter the 6-digit code sent to <span className="font-semibold text-gray-700">{email}</span></p>
      </div>
      ...
      >
        {isLoading ? <><Loader2 className="w-4 h-4 animate-spin" /> Verifying...</> : <>Verify & Login <ArrowRight className="w-4 h-4" /></>}
      </motion.button>
      <div className="flex items-center justify-between mt-4">
        <button onClick={onBack} className="text-sm text-gray-500 hover:text-gray-700 transition-colors">← Back to Login</button>
...
const ForgotPasswordStep = ({ onEmailSubmitted, onBack }: { onEmailSubmitted: (email: string) => void; onBack: () => void }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) { setError(data.message || "Failed to send reset link"); setIsLoading(false); return; }
  ...
  return (
    <motion.div initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }}>
      <div className="text-center mb-8">
        ...
        <p className="text-gray-500 text-sm mt-2">Enter your email and we'll send you a code to reset your password.</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <motion.button type="submit" disabled={isLoading}
          whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
          className="w-full py-3.5 bg-primary text-white font-semibold rounded-xl flex items-center justify-center gap-2 shadow-lg shadow-primary/20 text-sm mt-4"
        >
          {isLoading ? <><Loader2 className="w-4 h-4 animate-spin" /> Sending...</> : <>Send Reset Code <ArrowRight className="w-4 h-4" /></>}
        </motion.button>
      </form>
...
// ─── Reset Password Step ──────────────────────────────────────────────────────
const ResetPasswordStep = ({ email, otp, onResetComplete, onBack }: { email: string; otp: string; onResetComplete: () => void; onBack: () => void }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) { setError(data.message || "Failed to reset password"); setIsLoading(false); return; }
      onResetComplete();
    } catch { setError("An error occurred"); setIsLoading(false); }
  };
  ...
  return (
    <motion.div initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }}>
      ...
      <div className="mt-6 text-center">
        <button onClick={onBack} className="text-sm text-gray-500 hover:text-gray-700 transition-colors">Cancel</button>
      </div>
    </motion.div>
  );
};
    ...
    try {
      ...
      if (res?.error) { 
        if (res.error === "UnverifiedEmail") {
          // Send OTP and switch to OTP step
  ...
  return (
    <div className="min-h-screen flex pt-16">
      ...
      <motion.div initial={{ opacity: 0, x: -40 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }}
        className="hidden lg:flex flex-col justify-between w-[55%] bg-gradient-to-br from-[#061F33] via-[#0a2d4a] to-[#0d3a5c] p-12 relative overflow-hidden"
      >
        <HeroOrbs />
        ...
        <div className="relative z-10 space-y-8">
          <div>
            ...
            >
              Your License is{" "}
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary to-teal-300">One Step Away</span>
            </motion.h1>
            <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.75 }}
              className="text-white/60 text-lg mt-4 max-w-md leading-relaxed"
            >
              Join Canada's most trusted NPPE preparation platform. Expert-crafted content, practice tests, and 1-on-1 coaching.
            </motion.p>
          </div>
        ...
        <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.4 }} className="relative z-10 text-white/30 text-sm">
          ...
          <AnimatePresence mode="wait">
            {step === "form" ? (
              <motion.div key="form" variants={container} initial="hidden" animate="show" exit={{ opacity: 0, x: -30 }}>
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                  ...
                  <motion.div variants={item}>
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <button type="button" onClick={() => setStep("forgot-password")} className="text-xs font-semibold text-primary hover:text-primary/80">Forgot password?</button>
                    </div>
                    <div className="relative">
                      <Lock className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" />
                      <input type="password" value={password} onChange={(e) => setPassword(e.target.value)}
                ...
                <motion.p variants={item} className="text-center text-sm text-gray-500 mt-8">
                  Don't have an account?{" "}
                  <Link href="/signup" className="font-semibold text-primary hover:text-primary/80 transition-colors">Create one free →</Link>
                </motion.p>
              </motion.div>
            ) : step === "forgot-password" ? (
              <motion.div key="forgot-password" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }}>
                <ForgotPasswordStep 
              ...
              <motion.div key="reset-password" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }}>
                <ResetPasswordStep 
                  ...
                  onResetComplete={async () => {
                    ...
                    // But we don't have the new password stored in the parent component easily.
                    // Let's just redirect to login form and show a success message.
                    setStep("form");
              ...
              <motion.div key="otp" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }}>
                <OtpStep 
                  ...
                  onVerified={(code) => {
                    if (password === "") {
                      // If password is empty, they must be coming from the forgot password flow
                      setOtpCode(code || "");
                      setStep("reset-password");
                    } else {
                      // They are coming from standard login
                      handleOtpVerified();
                    }
                  }} 
                  onBack={() => setStep("form")} 
